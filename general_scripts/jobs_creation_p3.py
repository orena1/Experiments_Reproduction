...
def set_circuit_target_by_axes_limit(blue_config_circ, path_for_simulations, target_settings, base_target ='mc2_Column'):
    ...
    if base_target == 'mc2_Column': #need to change this eventually
        target_name ='mc2_X' + repr(target_settings['x'][0]).replace('-','m') + 'to'+ repr(target_settings['x'][1]).replace('-','m') +\
                        '_Y' + repr(target_settings['y'][0]).replace('-','m') + 'to'+ repr(target_settings['y'][1]).replace('-','m') +\
                        '_Z' + repr(target_settings['z'][0]).replace('-','m') + 'to'+ repr(target_settings['z'][1]).replace('-','m')

    else:
        target_name = base_target +  '_X' + repr(target_settings['x'][0]).replace('-','m') + 'to'+ repr(target_settings['x'][1]).replace('-','m') +\
                        '_Y' + repr(target_settings['y'][0]).replace('-','m') + 'to'+ repr(target_settings['y'][1]).replace('-','m') +\
                        '_Z' + repr(target_settings['z'][0]).replace('-','m') + 'to'+ repr(target_settings['z'][1]).replace('-','m')
    ...
    gid_to_x =list(circ.v2.cells.get(mc2_gids,properties=[Cell.X]).to_dict().values())[0]
    gid_to_y =list(circ.v2.cells.get(mc2_gids,properties=[Cell.Y]).to_dict().values())[0]
    gid_to_z =list(circ.v2.cells.get(mc2_gids,properties=[Cell.Z]).to_dict().values())[0]
    for gid in gid_to_x:
...
def create_blueconfig(BlueConfig_file, CurrentDir, BS, simulation_duration, run_name, ca, k, Mg,
                    circuit_target='mc2_Column', decouple=False, optogenetic_vars = [], RunMode = 'RunMode LoadBalance', DisableUseDep = [], reports={}, remove_spon_minis=False,
    ...
    for line in BlueConfig_file:
        if 'CurrentDir' in line:
            ...
            newF += '         Duration ' + repr(simulation_duration)+'\n'
...
def create_launch_script(launchScript_file, hoc_lib, init_name, special_path, simulation_time, run_name, nice_level=0,
                         nodes=512, ntask_per_node=32,cpus_per_task=None, bbpviz_txt = '', partition='prod',account='proj2', job_name='', core_neuron = False):
        ...
        if "--output=" in line:
            ...
            if  bbpviz_txt=='':
                if ntask_per_node:
                    ...
                    newF +='#SBATCH --cpus-per-task=' +str(cpus_per_task) +'\n'
            elif bbpviz_txt!='':
                if ntask_per_node:
                    newF +='#SBATCH --ntasks-per-node=' +str(ntask_per_node) +'\n'
                else:
            ...
            newF +='#SBATCH --nice=' + repr(nice_level)+'\n'
        elif "--error" in line:
            ...
            #srun dplace special BlueConfig_num_0 -mpi -python /gpfs/bbp.cscs.ch/home/amsalem/Dropbox/Blue_Brain/Experiments_Reproduction/Coupling_Coefficient/pylibs/manager_python_damus.py
...
def submit_jobs(run_names, paths_for_simulations, MaxJobs, all_after_one=False, ssh_path ='bbpv1.epfl.ch', force_all_after_one = False,
    ...
    for l,path_for_simulations in zip(run_names,paths_for_simulations):
        ...
        print(result)
        print(erre)
        JobsList.append(result[0].split()[-1].decode('ascii'))
        NumOfJobs+=1
