...
def measure_Rin_impedance(all_gids, CCs, Rins, gc = None):
    ...
    # With impedance tool
    h.node0.log('first gid = ' + str(all_gids[0]))
...
#non_sotchastic_mechs = ['TC_cad', 'TC_ih_Bud97', 'TC_Nap_Et2', 'TC_iA', 'TC_iL', 'TC_HH', 'TC_iT_Des98']
#sotchastic_mechs = []
...
#measure_Rin(all_gids, CCs, Rins, gc = 0.2);
#broadcast_Rins(gc=0.2, Rins=Rins)
#gc  = 0.2
#h.node0.log( "\nGJc " + str(gc) +\
    #' | mean norm Rin = ' + str([Rins[gc][gid][-1] for gid in all_gids]))
...
for gc in GJcs:
    ...
    h.node0.log( "\npre submit - GJc " + str(gc) +\
        ' | mean norm Rin = ' + str(Rins[gc]))
    broadcast_Rins(gc, Rins)
    h.node0.log( "\nafter submit - GJc " + str(gc) +\
        ' | mean norm Rin = ' + str([Rins[gc][gid][-1] for gid in all_gids]))
