...
run = 'random' # hubs | random | remove_none
...
if 'hubs' in run: 
    ...
    gids_to_remove = pickle.load(open('/gpfs/bbp.cscs.ch/project/proj2/simulations/Hubs_attack/data_from_Noam_29_04_2018/random_nodes_dict.pk1','r'))
    force_all_after_one=True
    all_after_one=False
...
for set_n in gids_to_remove:
    path_for_simulations = '/gpfs/bbp.cscs.ch/project/proj2/simulations/Hubs_attack/29_04_2018/Ca' + str(ca).replace('.','p') + '_K' + str(k).replace('.','p') +'/Run_' +str(run)  +  '/' + set_n +'/'
