...
if cluster == 'neuron_coreneuron':
    ...
    if phase_2:
        nodes = 30#55
        ntask_per_node = 40
    if knl == False and phase_2 == False:
        nodes = 96
        ntask_per_node = 36
...
def save_spikes(sp_per_axon_gid, save_path):
    ...
    [f.write('{:} \t {:} \n'.format(spike_time,int(axon_gid))) for spike_time,axon_gid in sps]
    f.close()
...
all_after_one=True
attack_source = 'hubs'#'betweeness'##'L6E-hubs'#'L2E-hubs'#,'L5E-hubs'#'L4E-hubs' #hubs, in-hubs, out-hubs, katz, pagerank
...
#in case that I want to take only a part:
#hubs_attacked = list(np.sort(hubs_attacked)[::8][:7]) + [np.sort(hubs_attacked)[-4]]

attack_to_simulate = hubs_attacked

files_to_simulate = [i for i in files if len([1 for j in attack_to_simulate if '{:05d}'.format(j)+'.(' in i])>0]#1
'''
#asds
#sdfs
...
#in case we want only random nodes
files = [i for i in files if '.random_nodes.' in i and 'txt' not in i]# and ('ihub' in i or 'ehub' in i)]

files = [i for i in files if '00001' in i]
files_to_simulate = files

#files_to_simulate = rnd.choice(files, size=70, replace =False)


#in case that I want to take only a part.. only random_nodes
#files = np.sort([i for i in files if 'random_nodes.' in i])[::4][::8][::3]
#files_to_simulate = files
...
#for vari in [1.5,4]:
    #for axon_stim_firing_rate in [1,3, 7]:
        #for axon_spontanues_firing_rate in [1,2.5]:
            #factor = create_normalization_factor(vari)
phase = 0
freq = 1
axon_spontanues_firing_rate = 1
axon_stim_firing_rate = 7
vari = 1.5
mod_freq1 = freq * 2  * np.pi/1000.0
factor = create_normalization_factor(vari)
thalmaic_input = False
thalamic_path = ''
if thalmaic_input:
    ...
    raw_input("Using_thalamic input!, 5 seeds!")
    
print(files_to_simulate)
running = 0
for attack in files_to_simulate:
    clean_attack = attack.replace('(','_').replace(')','_')[:-3]
    run_names = []
    paths_for_simulations = []
    path_for_simulations = '/gpfs/bbp.cscs.ch/project/proj2/simulations/Reproducing_Experiments/29_7_19/' + thalamic_path + 'Ca' + str(ca).replace('.','p') + '_K' + \
    ...
    for seed in seeds:
        ...
        if not os.path.exists(path_for_simulations + '/' + run_name +'/out.dat'): # if output already exists, do not send sim
    ...
    if running<100:
        jobs_lists = submit_jobs(run_names, paths_for_simulations, MaxJobs = 10,  all_after_one=all_after_one, ssh_path=ssh_path, force_all_after_one=force_all_after_one)
    else:
        jobs_lists = submit_jobs(run_names, paths_for_simulations, MaxJobs = 10,  all_after_one=all_after_one, ssh_path=ssh_path, force_all_after_one=force_all_after_one,
                                 first_dependecy = jobs_lists[-1])
    running+=1
    #sfdsd