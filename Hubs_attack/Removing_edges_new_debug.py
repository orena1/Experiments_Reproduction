...
if cluster == 'neuron_coreneuron':
    ...
    if knl == True:
        partition = 'prod_knl'
        bbpviz_txt ='''
    export ARCHIVE_MODULE_DATE="2019-09"
    ...
    '''
        ...
        bbpviz_txt+=  'module load neurodamus-neocortex/0.2/knl/python3/parallel/serial \n . /gpfs/bbp.cscs.ch/home/amsalem/damus_venv/bin/activate'
    hoc_lib = ''
    init_name = ''
...
all_after_one=True
net_path = '/gpfs/bbp.cscs.ch/data/project/proj5/hub_attacks/networks_debug/'
...
### random sample
rnd = np.random.RandomState(170) #112;70, 170;70, 70;45, 150;15 10,20 , 50, 35,120
files = [i for i in files if 'edges' in i]
files_to_simulate = rnd.choice(files, size=5, replace =False)
...
for attack in files_to_simulate:
    ...
    path_for_simulations = '/gpfs/bbp.cscs.ch/project/proj2/simulations/Reproducing_Experiments/29_7_19/Ca' + str(ca).replace('.','p') + '_K' + str(k).replace('.','p') +'/debug_remove_edges_Run_' +str(run)  +  '/' + clean_attack +'/'
