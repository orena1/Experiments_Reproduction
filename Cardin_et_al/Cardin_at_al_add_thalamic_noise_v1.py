...
simulation_time = "10:00:00" #It has to be 03 or 05 etc..
nice_level = 0
...
for seed in seeds:
    for amp in amplitudes:
        for pulse_width in pulse_widths:
            for freq in freqs:
                for pulses_fr in [5]:#,10,20]:
                    for fr in [0.0001]:#,0.2,0.5]:
                        dur = stim_durations
                        BS = seed*1000000 + int(pulses_fr*1214) +  int(amp*10) + int(pulse_width*10000) + int(freq*1000) + int(fr*800)

                        sp_per_axon_gid = create_sync_thalamic_spikes(pulses_fr,  fr, 1000, simulation_duration-250, BS )
