...
if cluster == 'bbpviz1':
    ...
    base_dir = '/gpfs/bbp.cscs.ch/project/proj2/Programs/Master_Core_20_9_18/soft'
    if change_gamma:
...
groups = 'loc_based'
run = 'check_for_fano_factor_n2_knl'
...
def create_sync_thalamic_spikes(pulses_fr, fr_per_axon, stim_start_time, stim_end_time, seed):
    ...
    filtered_fr = scipy.ndimage.filters.gaussian_filter(fr_per_bin, 2/0.025)
    time_to_fr = {round(bin_times[i],3):filtered_fr[i] for i in range(len(bin_times[:-1]))}
...
path_for_simulations = '/gpfs/bbp.cscs.ch/ssd/tmp_compile/amsalem/simulations/Reproducing_Experiments/Cardin_Nature_2009/23_07_2018/Ca' + \
...
for seed in seeds:
    for amp in amplitudes:
        for pulse_width in pulse_widths:
            for freq in freqs:
                for fr in [0.1,2,8,32]:
                    dur = stim_durations
                    BS = seed*1000000 + int(amp*10) + int(pulse_width*10000) + int(freq*1000) + int(fr*800)

                    sp_per_axon_gid = create_thalamic_spikes(fr, 1000, simulation_duration-250, BS )
                    spike_replay_path = path_for_simulations + '/SpikeFiles/input' + str(float(fr)).replace('.','p') + '_BS' + `BS`
                    save_spikes(sp_per_axon_gid, spike_replay_path)


                    run_name = 'fr_' + `float(fr)`.replace('.','p') + 'freq' + `float(freq)`.replace('.','p') +  '_amp' + `float(amp)`.replace('.','p') + '_pulse_width' + `float(pulse_width)`.replace('.','p') + '_BS' + `BS`
