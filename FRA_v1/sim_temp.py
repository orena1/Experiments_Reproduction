...
tm_text = ''
tm_text1 = ''
home_fold = '/gpfs/bbp.cscs.ch/home/amsalem/rsync/Blue_Brain'
exec(compile(open(f'{home_fold}/OSI_files/CreateSimFiles_GitHub/CreateAxonalSpikeFilePossionGenerator_clean_auditory.py', "rb").read(), f'{home_fold}/OSI_files/CreateSimFiles_GitHub/CreateAxonalSpikeFilePossionGenerator_clean_auditory.py', 'exec'))
exec(compile(open(f'{home_fold}/Experiments_Reproduction/general_scripts/jobs_creation_p3.py', "rb").read(), f'{home_fold}/Experiments_Reproduction/general_scripts/jobs_creation_p3.py', 'exec'))
exec(compile(open(f'{home_fold}/Experiments_Reproduction/general_scripts/opto_gen_protocols.py', "rb").read(), f'{home_fold}/Experiments_Reproduction/general_scripts/opto_gen_protocols.py', 'exec'))
...
BasePath = '/gpfs/bbp.cscs.ch/project/proj2/simulations/ThlInput/AudInput28_11_22/SSA/'
if change_gamma==False:
...
projection_path = '/gpfs/bbp.cscs.ch/project/proj2/circuits/Projections/proj32/BlobStim/proj_test_O1/s2f'

#projection_path = '/gpfs/bbp.cscs.ch/project/proj2/circuits/Thalamic_for_FRA/Mosaic/MaxiHat_strength3_a4_change_condTrue_rsFalse_freq10079/'
...
#AxoGidToXY = pickle.load(open(os.environ['HOME'] + '/AxoGidToXY_O1.pkl','rb'),encoding='latin1')
...
if 'axon_gid_to' in axons_path:
    BasePath = BasePath.replace('FRA','O1_slice_mc2/FRA')
    ...
    BasePath = BasePath.replace('FRA','O1_slice_mc2_all_axons/FRA')
else:
...
#edge_frequencies = [(1333, 48000)] # this is what was here before 2022, I'm changing it as a function of FRA/check_axons_responses.ipynb
edge_frequencies = [(2700, 20300)] # I'm changing it as a function of FRA/check_axons_responses.ipynb
...
experiment['firing_rate_sample_seed'] = 0
if experiment['firing_rate_sample_seed'] != 0:
    input("I am changing the seed of the firing rate sample, it will get different results!")
experiment['responsive_sample_seed'] = 0
if experiment['responsive_sample_seed'] != 0:
    input("I am changing the seed of the responsive_sample_seed it will get different results!")


SSA = 0 
if __name__ == '__main__':
    for amp in [30,40,50,60,70,80]:#40,50,60,70,80]:
        set_exp_by_amp(amp)
        # for x_start in [250]:
                            ...
                            for Disable_CortoCortical in Disable_CortoCorticals:
                                for seed in [1,2,3,4,5]:
                                    for freq_start,freq_end in edge_frequencies:
                                        ...
                                        if os.path.exists(path_for_simulations) and freq_start == [6666,9600]:tm_text1 = input('This path already exist (pass)'); 
                                        ...
                                        spike_replay = path_for_simulations + '/SpikeFiles/input' + str(freq_start) + '_' + str(freq_end) + '_' + repr(BS) + '.dat'
                                        print(spike_replay)

                                        run_name = str(freq_start) + '_' +str(freq_end)  +'_'+ repr(BS) 
                                        
                                        ## need to use this one.
                                        job_name =  'S' + repr(int(freq_start)) +  '_E' + repr(int(freq_end)) +  '_' + path_for_simulations[path_for_simulations.index('/Ca')-5:]
